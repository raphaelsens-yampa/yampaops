...
export function StageManager({ stages, onUpdate }: Props) {
  ...
  function openCreate() {
    setIsNew(true); setName(""); setSlug(""); setColor("#94a3b8"); setIsWon(false); setIsLost(false);
    setEditStage({} as PipelineStage);
  }

  function openEdit(s: PipelineStage) {
    setIsNew(false); setName(s.name); setSlug(s.slug); setColor(s.color || "#94a3b8");
    setIsWon(s.is_won); setIsLost(s.is_lost); setEditStage(s);
  }
  ...
  async function handleSave() {
    ...
    if (isNew) {
      ...
      const { error } = await supabase.from("pipeline_stages").insert({ name: name.trim(), slug: finalSlug, position: maxPos, color, is_won: isWon, is_lost: isLost });
      if (error) toast({ title: "Erro ao criar etapa", description: error.message, variant: "destructive" });
      else toast({ title: "Etapa criada" });
    } else if (editStage?.id) {
      const { error } = await supabase.from("pipeline_stages").update({ name: name.trim(), slug: finalSlug, color, is_won: isWon, is_lost: isLost }).eq("id", editStage.id);
      if (error) toast({ title: "Erro ao atualizar", description: error.message, variant: "destructive" });
      else toast({ title: "Etapa atualizada" });
    }

    setSaving(false); setEditStage(null); onUpdate();
  }

  async function handleDelete(s: PipelineStage) {
    const { count } = await supabase.from("opportunities").select("id", { count: "exact", head: true }).eq("stage", s.slug);
    if (count && count > 0) {
      toast({ title: "Não é possível excluir", description: `Existem ${count} oportunidades nesta etapa. Mova-as antes de excluir.`, variant: "destructive" });
      return;
    }
    const { error } = await supabase.from("pipeline_stages").delete().eq("id", s.id);
    if (error) toast({ title: "Erro ao excluir", description: error.message, variant: "destructive" });
    else { toast({ title: "Etapa excluída" }); onUpdate(); }
  }
  ...
  return (
    <div className="space-y-4">
            ...
            <TableRow key={s.id}>
              <TableCell>
                <div className="flex flex-col gap-0.5">
                  <Button variant="ghost" size="icon" className="h-5 w-5" disabled={idx === 0} onClick={() => moveStage(s, "up")}><ArrowUp className="h-3 w-3" /></Button>
                  <Button variant="ghost" size="icon" className="h-5 w-5" disabled={idx === stages.length - 1} onClick={() => moveStage(s, "down")}><ArrowDown className="h-3 w-3" /></Button>
                </div>
              </TableCell>
              <TableCell><div className="w-5 h-5 rounded" style={{ backgroundColor: s.color || "#94a3b8" }} /></TableCell>
      ...
      <Dialog open={!!editStage} onOpenChange={(open) => !open && setEditStage(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>{isNew ? "Nova Etapa" : "Editar Etapa"}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1">
              <Label>Nome</Label>
              <Input value={name} onChange={e => { setName(e.target.value); if (isNew) setSlug(generateSlug(e.target.value)); }} placeholder="Ex: Qualificação" />
            </div>
            <div className="space-y-1">
              <Label>Slug (identificador)</Label>
              <Input value={slug} onChange={e => setSlug(e.target.value)} placeholder="Ex: qualificacao" className="font-mono text-sm" />
            </div>
            <div className="space-y-1">
              <Label>Cor</Label>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={saving || !name.trim()}>{saving ? "Salvando..." : isNew ? "Criar" : "Salvar"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
