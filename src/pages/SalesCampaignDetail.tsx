...
import { ArrowLeft, Upload, RefreshCw, Plus, Trash2, Save, Pencil, X, MessageCircle, CheckCircle2, Circle, Eraser, Bot, User, Download } from "lucide-react";
...
function CohortTab({ campaign }: { campaign: Campaign }) {
  ...
  const qc = useQueryClient();
  const sync = useCohortSync();
  const isSyncingThis = sync.syncing && sync.campaignId === campaign.id;
  ...
  // Refetch when the global sync finishes for this campaign
  const lastPhase = useRef(sync.phase);
  useEffect(() => {
    if (lastPhase.current !== sync.phase && sync.phase === "done" && sync.campaignId === campaign.id) {
      refetch();
    }
    lastPhase.current = sync.phase;
  }, [sync.phase, sync.campaignId, campaign.id, refetch]);
