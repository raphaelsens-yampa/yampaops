CREATE OR REPLACE FUNCTION public.resolve_stripe_seller(p_customer_id text, p_email text, p_at timestamp with time zone DEFAULT now(), p_price_id text DEFAULT NULL::text)
...
DECLARE
  v_seller uuid;
BEGIN
  -- Única fonte de verdade: Mapa de Preços (commission_price_map)
...
CREATE OR REPLACE FUNCTION public.resolve_stripe_seller(p_customer_id text, p_email text, p_at timestamp with time zone DEFAULT now())
 RETURNS TABLE(seller_id uuid, source text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  -- Sem price_id não há como atribuir: Mapa de Preços é a única fonte
  seller_id := NULL; source := NULL; RETURN NEXT;
END;
$function$;
...
BEGIN
  ...
  ELSE
    ...
    ELSE
      ...
      END IF;
    END IF;
    -- Mapa de Preços é a única fonte de vendedor
