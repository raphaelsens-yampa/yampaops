...
BEGIN
  ...
  IF v_map.id IS NULL THEN
    ...
    -- Prioridade: mrr líquido da invoice > mrr bruto do price.
  ...
  ELSE
    ...
    -- 1) mrr_net (valor efetivamente cobrado, já com cupom) — fonte de verdade;
    -- 2) mrr_override do mapa — fallback usado apenas quando não há invoice paga;
    -- 3) mrr bruto do price — última opção.
    IF v_base_mode = 'net' THEN
      v_mrr := COALESCE(v_sc.mrr_net, v_map.mrr_override, v_sc.mrr, 0);
    ELSE
      -- Modo 'gross': mantém comportamento antigo (override manda, senão mrr bruto)
